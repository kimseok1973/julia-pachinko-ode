...
| ファイル | 説明 |
|---------|------|
| [`パチンコの微分方程式モデリング2026.ipynb`](パチンコの微分方程式モデリング2026.ipynb) | メインノートブック（42セル） |
...
| 2 | データ駆動人口変動 | 実人口データを補間して $\dot{V}_u(t)$ を推定 |
| 3 | ベイズ推定（全期間） | NUTS サンプラーで $e$, $r$ を一括推定・WAIC 計算 |
| 4 | カタストロフィーモデル | 離散イベントで需要ショックを組み込み・ベイズ推定 |
| 5 | 生産年齢人口モデル | $V$ を総人口から生産年齢人口に差し替えて再推定 |
| 6 | 2030 年予測分布 | 人口 $-900$ 千人/年 を仮定し外挿・事後サンプルで 90% 確信区間を算出 |
